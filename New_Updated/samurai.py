    ...
    def _evaluate_model(self, y_true: np.ndarray, y_pred: np.ndarray, 
        ...
        if y_pred_proba is not None:
            ...
            try:
                ...
                plt.figure(figsize=(8, 6))
                plt.plot(fpr_curve, tpr_curve, label=f'{model_name} (AUC = {metrics["auc"]:.3f})')
                plt.plot([0, 1], [0, 1], 'k--')
                plt.xlabel('False Positive Rate')
                plt.ylabel('True Positive Rate')
                plt.title(f'ROC Curve - {model_name}')
        ...
        if cm.shape == (2, 2):
            ...
            print(f"  │  Actual                                  │")
            print(f"  │  Clean      TN={tn:<6}   FP={fp:<6}          │")
            print(f"  │  Adversarial FN={fn:<6}   TP={tp:<6}          │")
            print(f"  └─────────────────────────────────────────┘")
            print(f"")
            print(f"  True  Positives (TP) : {tp:<6}  ← attacks correctly caught")
            print(f"  True  Negatives (TN) : {tn:<6}  ← clean correctly passed")
            print(f"  False Positives (FP) : {fp:<6}  ← clean flagged as attack")
            print(f"  False Negatives (FN) : {fn:<6}  ← attack missed!")
